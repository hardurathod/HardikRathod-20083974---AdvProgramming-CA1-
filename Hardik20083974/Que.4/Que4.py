...
with open("hotelA.html", "r", encoding="utf-8") as f:
    html = f.read()
block = html.split("const rooms = [")[1].split("];")[0]
rooms = block.split("},")
for r in rooms:
    name = r.split("name: '")[1].split("'")[0]
    price = r.split("price: ")[1].split(",")[0]
...
with open("hotelB.html", "r", encoding="utf-8") as f:
    ...
    for room in rooms:
        ...
        price_tag = room.find("div", class_="price")  
        if not price_tag:
            price_tag = room.find("div", class_="price-tag") 
